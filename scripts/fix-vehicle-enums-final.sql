-- Script para corregir TODOS los valores enum vacíos en la tabla Vehicle
...
SELECT
    ...
    status
FROM Vehicle
WHERE fuelType = '' OR transmission = '' OR status = '' 
   OR fuelType IS NULL OR transmission IS NULL OR status IS NULL;
...
-- 4. Corregir valores vacíos o NULL para status
UPDATE Vehicle
...
-- 6. Verificar que todos los vehículos tienen valores válidos
SELECT
