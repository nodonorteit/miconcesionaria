-- Script para arreglar templates duplicados y constraint único
-- Error: "Unique constraint failed on the constraint: document_templates_name_key"

-- 1. Verificar todos los templates existentes
SELECT 
    id,
    name,
    type,
...
-- 2. Identificar templates duplicados por nombre
SELECT 
    name,
    COUNT(*) as count,
...
-- 3. Eliminar templates duplicados (mantener el más reciente)
DELETE dt1 FROM document_templates dt1
INNER JOIN document_templates dt2 
WHERE dt1.name = dt2.name 
AND dt1.id != dt2.id 
AND dt1.createdAt < dt2.createdAt;

-- 4. Verificar que no quedaron duplicados
SELECT 
    name,
    COUNT(*) as count
FROM document_templates
...
-- 5. Asegurar que hay al menos un template por defecto
...
<body>
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM document_templates WHERE name = 'Boleto Estándar'
);
